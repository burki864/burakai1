  ...
  const themes: Record<ThemeType, React.ReactNode> = {
    ...
    rain: (
      <div className="fixed inset-0 pointer-events-none z-[-1] overflow-hidden bg-slate-950">
        ...
        {[...Array(60)].map((_, i) => (
          <div 
            key={i} 
            className="absolute bg-blue-400/30 w-[1px] h-14 rounded-full animate-rain"
            style={{ 
        ...
        {[...Array(20)].map((_, i) => (
          <div 
            key={`s-${i}`} 
            className="absolute bottom-0 bg-blue-300/20 w-1 h-1 rounded-full animate-splash"
            style={{ 
              left: `${Math.random() * 100}%`,
    ...
    desert: (
      <div className="fixed inset-0 pointer-events-none z-[-1] bg-[#0c0502] overflow-hidden">
        ...
        {/* Heat haze effect */}
        <div className="absolute inset-0 opacity-20 bg-[url('https://www.transparenttextures.com/patterns/sand.png')] animate-sand-drift"></div>
        ...
        <div className="absolute top-10 right-10 w-40 h-40 bg-orange-500/5 rounded-full blur-[80px]"></div>
      </div>
    ),
    nebula: (
      <div className="fixed inset-0 pointer-events-none z-[-1] bg-[#02020a] overflow-hidden">
        <div className="absolute top-[-10%] right-[-10%] w-[80%] h-[80%] bg-purple-900/10 rounded-full blur-[150px] animate-nebula-pulse"></div>
        <div className="absolute bottom-[-10%] left-[-10%] w-[70%] h-[70%] bg-blue-900/10 rounded-full blur-[150px] animate-nebula-pulse-delayed"></div>
        {/* Twinkling stars */}
        {[...Array(100)].map((_, i) => (
          <div 
            key={i} 
    ...
    cyberpunk: (
      <div className="fixed inset-0 pointer-events-none z-[-1] bg-[#010105] overflow-hidden">
         ...
         <div className="absolute inset-0 bg-[linear-gradient(rgba(0,255,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(0,255,255,0.03)_1px,transparent_1px)] bg-[size:40px_40px] [transform:perspective(1000px)_rotateX(60deg)_translateY(-100px)_scale(2)] animate-grid-drift"></div>
         <div className="absolute inset-0 bg-gradient-to-t from-transparent via-cyan-500/5 to-transparent h-px w-full top-1/2 animate-scan"></div>
         {/* Neon flares */}
         <div className="absolute top-0 left-1/4 w-[1px] h-full bg-magenta-500/10 blur-sm shadow-[0_0_20px_rgba(255,0,255,0.2)]"></div>
         <div className="absolute bottom-1/4 right-0 h-[1px] w-full bg-cyan-500/10 blur-sm"></div>
      </div>
    )
  };
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes sand-drift { from { background-position: 0 0; } to { background-position: 100% 100%; } }
        ...
        @keyframes grid-drift { from { background-position: 0 0; } to { background-position: 0 40px; } }
        ...
        .animate-grid-drift { animation: grid-drift 2s linear infinite; }
